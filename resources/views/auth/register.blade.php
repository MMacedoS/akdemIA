...
@section('content')
    <style>
        ...
        .manual-register-shell {
            display: grid;
            gap: 14px;
        }

        .manual-register-note {
            margin: 0;
            padding: 14px 16px;
            border-radius: 18px;
            background: #f3f6f4;
    ...
    @if (!empty($canLoginWithGoogle))
        <div class="social-auth-stack">
            ...
            <div class="social-auth-divider">cadastro manual opcional</div>
        </div>
    @endif

    <div class="manual-register-shell">
        @if (!empty($canLoginWithGoogle))
            <p class="manual-register-note">Se preferir, voce ainda pode criar a conta manualmente com e-mail e senha. O fluxo com Google continua sendo o principal.</p>
        @endif
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <button type="submit" class="auth-btn">Cadastrar manualmente</button>
        </form>
    </div>
