...
@section('headerTitle', 'Visao do Trainee')
...
@section('headerAction')
    <div class="actions">
        ...
        @endif
        <a class="btn btn-soft" href="{{ route('trainee.students.edit', $student->id) }}">Editar saude</a>
    </div>
@endsection
...
@section('content')
    ...
    <div class="content-stack trainee-student-show">
        ...
        @if ($canManageWorkouts && ($availableCatalogsToApply ?? collect())->isNotEmpty())
            <section class="card compact-section">
                <form
                    method="POST"
                    action="{{ route('trainee.students.workouts.catalog.apply', [$student->id, 0]) }}"
                    data-action-base="{{ route('trainee.students.workouts.catalog.apply', [$student->id, 0]) }}"
        ...
        @if (($availableAppliedCatalogs ?? collect())->isNotEmpty())
            <section class="card compact-section">
                <form method="GET" action="{{ route('trainee.students.show', $student->id) }}" class="catalog-filter-form">
                    ...
                    <div class="catalog-filter-row">
                        ...
                        @if (($selectedCatalogId ?? null) !== null)
                            <a href="{{ route('trainee.students.show', $student->id) }}" class="btn btn-soft">Limpar</a>
                        @endif
                    </div>
                </form>
            </section>
        ...
        <section class="card compact-section">
            ...
            @if (! $canManageWorkouts)
                <div class="empty-state">
                    <p>Selecione um tenant para gerar e acompanhar treinos deste aluno.</p>
                </div>
            @endif
                ...
                <div class="workout-grid generated-grid">
                    @foreach ($generatedWorkouts as $index => $workout)
                        ...
                        <article class="workout-item generated {{ $requestStatus === 'active' ? 'is-active' : 'is-inactive' }}">
                            ...
                            <div class="actions">
                                <a class="btn btn-primary" href="{{ route('trainee.students.workouts.show', [$student->id, $workout->id]) }}">Abrir board</a>
                            </div>
                        </article>
                    @endforeach
                </div>
    ...
    <style>
        .trainee-student-show {
            gap: 16px;
        }

        .trainee-student-show .student-hero {
            display: grid;
        ...
        .trainee-student-show .student-hero h3 {
            margin-bottom: 6px;
            font-size: 21px;
        }
    ...
    @if ($canManageWorkouts)
        <div id="generate-workout-modal" class="plan-refazer-modal" aria-hidden="true">
            <div class="plan-refazer-modal-card">
                ...
                <form method="POST" action="{{ route('trainee.students.workouts.generate', $student->id) }}" class="content-stack">
                    @csrf
